function [theta,result_train,result_test,result_train_k,result_test_k] ...
    = Algo_cv(data,pms,graph,RF,weight)
%ALGO_CV
%   Version:2022/07/14
...
for j=1:pms.J
    ...
    % For \mathcal{N}_j + {j}.(0908)
    inv_temp{j} = inv_temp{j}+1*2*c_j{j}*z{j}*z{j}';
...
for iter=1:iter_max
    for j=1:pms.J
        ...
        % For \mathcal{N}_j + {j}.(0908)
        theta_temp{j} = theta_temp{j}+1*2*c_j{j}*z{j}*z{j}'*theta{j};
    end
    for j=1:pms.J
