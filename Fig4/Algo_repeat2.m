function [Algo_result,result_Algo_node_test] = Algo_repeat2(data,pms,graph,repeat_time)
...
D_all = pms.J*pms.D_j;
for j=1:pms.J
%     weight(j,1) = 1; % For equal D_j.
    weight(j,1) = sqrt(data.N_train(j)); % For different D_j.
end
...
result_Algo_node_train = mean(Algo_node_train,2);
result_Algo_node_test = mean(Algo_node_test,2);
