function [DKLA_result,DKLA_result_node] = DKLA_repeat(data,pms,graph,repeat_time) 
%DKLA_REPEAT
...
result_DKLA_node_train = mean(DKLA_node_train,2);
result_DKLA_node_test = mean(DKLA_node_test,2);

DKLA_result = [result_DKLA_train result_DKLA_test ... 
               std(DKLA_train_list) std(DKLA_test_list)];
                 
DKLA_result_node = [result_DKLA_node_train result_DKLA_node_test std(DKLA_node_train,0,2) std(DKLA_node_test,0,2)];
end